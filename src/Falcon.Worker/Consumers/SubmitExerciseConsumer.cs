...
public class SubmitExerciseConsumer : IConsumer<ISubmitExerciseCommand>
{
    ...
    public async Task Consume(ConsumeContext<ISubmitExerciseCommand> context)
    {
        ...
        _logger.LogInformation(
            "Processing exercise submission: Correlation {CorrelationId}, Group {GroupId}, Exercise {ExerciseId}",
        ...
        {
            // Load entities
            var exercise = await _dbContext.Exercises.FirstOrDefaultAsync(e =>
                e.Id == message.ExerciseId
            );

            var group = await _dbContext.Groups.FirstOrDefaultAsync(g => g.Id == message.GroupId);

            var competition = await _dbContext.Competitions.FirstOrDefaultAsync(c =>
                c.Id == message.CompetitionId
            );

            if (exercise == null || group == null || competition == null)
            {
                await context.Publish<ISubmitExerciseResult>(
                    new
                    {
                        message.CorrelationId,
            ...
            // Call Judge Service
            JudgeSubmissionResult judgeResult = await _judgeService.SubmitCodeAsync(
                message.Code,
            ...
            DomainJudgeResponse domainStatus = (DomainJudgeResponse)(int)judgeResult.Status;
            ...
            if (attempt.Accepted)
            {
                var ranking = await _dbContext.CompetitionRankings.FirstOrDefaultAsync(r =>
                    r.CompetitionId == message.CompetitionId && r.GroupId == message.GroupId
                );
                ...
                bool alreadySolved = await _dbContext.GroupExerciseAttempts.AnyAsync(a =>
                    a.GroupId == message.GroupId
            ...
            // Create log
            Log log = new Log(LogType.SubmitExercise, "worker", null, group, competition);
            await _dbContext.Logs.AddAsync(log);

            await _dbContext.SaveChangesAsync();

            // Recalculate rank order
            List<CompetitionRanking> rankings = await _dbContext
                .CompetitionRankings.Where(r => r.CompetitionId == message.CompetitionId)
            ...
            int solvedCount = await _dbContext
            ...
            _logger.LogInformation(
                "Exercise submission processed successfully: Correlation {CorrelationId}, Accepted: {Accepted}",
                message.CorrelationId,
                attempt.Accepted
            );
        }
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error processing exercise submission: Correlation {CorrelationId}",
                message.CorrelationId
            );
