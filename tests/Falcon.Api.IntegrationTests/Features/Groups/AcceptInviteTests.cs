...
public class AcceptInviteTests : TestBase
{
    public AcceptInviteTests(CustomWebApplicationFactory factory) : base(factory)
    {
    }

    [Fact]
    public async Task Should_ReturnSuccess_When_AcceptingInvite()
    {
        // Arrange
        ...
        // Act
        var response = await HttpClient.PostAsync($"/api/Group/invite/{invite.Id}/accept", null);
    ...
    public async Task Should_ReturnUnauthorized_When_NotAuthenticated()
    {
        ...
        // Act
        var response = await HttpClient.PostAsync($"/api/Group/invite/{Guid.NewGuid()}/accept", null);
